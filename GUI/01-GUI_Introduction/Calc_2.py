...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def calc(self):
        # self.sender() will detect that on which button you have clicked
        btn = self.sender()
        # retrieve text from that button on which click event is performed
        opr = btn.text()
        self.num_1 = self.lineEdit.text()
        self.num_2 = self.lineEdit_2.text()
        self.expression = self.num_1 + opr + self.num_2
